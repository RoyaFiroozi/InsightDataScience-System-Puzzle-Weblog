...
def index():
    ...
    sql_local = """SELECT COUNT(*) FROM weblogs WHERE source LIKE \'local\' AND status LIKE \'2__\';""" 
    cur.execute(sql_local)
    local = cur.fetchone()[0]

    # Get number of all remote requests
    sql_remote = """SELECT COUNT(*) FROM weblogs WHERE source LIKE \'remote\' AND status LIKE \'2__\';""" 
    ...
    if all != 0:
        ...
        local_rate = str(local / success)
        remote_rate = str(remote / success)
